...
Frame move(uint8_t address, uint8_t direction, uint16_t rpm,
    ...
    Frame frame = {{address, 0xFD, direction, 0x00, 0x00, acceleration, 0x00,
                    0x00, 0x00, 0x00, static_cast<uint8_t>(absolute),
                    static_cast<uint8_t>(start), kChecksum},
                   13,
                   0xFD};
    putUint16(&frame.bytes[3], rpm);
...
Frame stop(uint8_t address, Start start) {
    Frame frame = {
        {address, 0xFE, 0x98, static_cast<uint8_t>(start), kChecksum}, 5, 0xFE};
    return frame;
}

Frame home(uint8_t address, HomeMode mode, Start start) {
    Frame frame = {{address, 0x9A, static_cast<uint8_t>(mode),
                    static_cast<uint8_t>(start), kChecksum},
                   5,
                   0x9A};
    return frame;
...
Error validateResponse(const uint8_t* response, size_t size, uint8_t address,
                       uint8_t function) {
    if (size < 3 || response[0] != address || response[1] != function ||
        response[size - 1] != kChecksum) {
